...
In many situations, it is useful to be able to programatically specify not just one hierarchy configuration, but a set of valid configurations. For example, [OME-NGFF](https://ngff.openmicroscopy.org/latest/), a popular Zarr-based community file format for microscopy, specifies a configuration of groups and arrays for high-content screening experiments in which nested groups represent plates rows, and wells. In cases like these, a user preparing such data, or an application consuming such data, might want to check whether the incoming data conform to the specification. Such communities would benefit from the use of schemas as a rigorous way to represent the set of allowable hierarchy configurations.
...
TODO: WE SHOULD HAVE AN EXAMPLE OF AN ENFORCEABLE, SPECIFIC SCHEMA

TODO: IS IT POSSIBLE TO MAKE RULES ABOUT ALLOWABLE NODE NAMES?
